...
static void tagged_method_instance_root(rle_reference *rr, jl_ircode_state *s, int i)
{
    if (!get_method_instance_root_reference(rr, s->method_instance, i))
        s->relocatability = 0;
}
...
static void literal_val_method_instance_id(rle_reference *rr, jl_ircode_state *s, jl_value_t *v) JL_GC_DISABLED
{
    jl_array_t *rs = s->method_instance->roots;
    int i, l = jl_array_len(rs);
    for (i = 0; i < l; i++) {
        if (jl_egal(jl_array_ptr_ref(rs, i), v))
            return tagged_method_instance_root(rr, s, i);
    }
    jl_add_method_instance_root(s->method_instance, jl_precompile_toplevel_module, v);
    return tagged_method_instance_root(rr, s, jl_array_len(rs) - 1);
}
...
static void jl_encode_as_indexed_method_instance_root(jl_ircode_state *s, jl_value_t *v)
{
    assert(jl_is_method_instance(v));

    rle_reference rr;

    literal_val_method_instance_id(&rr, s, v);
...
static void jl_encode_value_(jl_ircode_state *s, jl_value_t *v, int as_literal) JL_GC_DISABLED
{
    ...
    if (tag != HT_NOTFOUND) {
        ...
        for (i = 0; i < jl_datatype_nfields(jl_lineinfonode_type); i++) {
            ...
            if (jl_is_method_instance(vi))
                jl_encode_as_indexed_method_instance_root(s, vi);
            else
...
static jl_value_t *jl_decode_value(jl_ircode_state *s) JL_GC_DISABLED
{
    ...
    case TAG_RELOC_METHODINSTROOT:
        ...
        assert(index >= 0);
        tag = read_uint8(s->s);
        assert(tag == TAG_RELOC_METHODROOT || tag == TAG_METHODROOT || tag == TAG_LONG_METHODROOT);
        int key_name = 0;
        if (tag == TAG_RELOC_METHODROOT) {
            key_name = read_uint64(s->s);
            tag = read_uint8(s->s);
            assert(tag == TAG_METHODROOT || tag == TAG_LONG_METHODROOT);
        }
        int index_name = -1;
        if (tag == TAG_METHODROOT)
            index_name = read_uint8(s->s);
        else if (tag == TAG_LONG_METHODROOT)
            index_name = read_uint32(s->s);
        assert(index_name >= 0);
        if (s->method_instance != NULL)
            return lookup_method_instance_root(s->method_instance, key, index);
        else
            return lookup_root(s->method, key_name, index_name);
    case TAG_METHODINSTROOT:
        ...
        assert(index_name >= 0);
        if (s->method_instance != NULL)
            return lookup_method_instance_root(s->method_instance, 0, index);
        else
            return lookup_root(s->method, 0, index_name);
