...
{
    // func may be jl_symbol, jl_method_t, or jl_method_instance_t
    ...
    for (i = 0; i < n; i++) {
        ...
        jl_value_t *ln_func = (jl_is_long(inl) && jl_unbox_long(inl) == 0) ? func : jl_fieldref_noalloc(ln, 1);
...
JL_DLLEXPORT jl_code_info_t *jl_code_for_staged(jl_method_instance_t *linfo)
{
    ...
    JL_TRY {
        ...
        jl_add_function_to_lineinfo(func, (jl_value_t*)linfo);
    }
    JL_CATCH {
        ct->ptls->in_pure_callback = last_in;
        jl_lineno = last_lineno;
...
static void jl_method_set_source(jl_method_t *m, jl_code_info_t *src)
{
    ...
    jl_add_function_to_lineinfo(src, (jl_value_t*)m);
