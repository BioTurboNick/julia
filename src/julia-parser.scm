...
(define (parse-ndim-matrix s first closer gotnewline last-end-symbol)
  (define (fix head v) (cons head (reverse v)))
  (define (nthcdr i l) ; get the list i elements back
    (if (<= i 0)
      l
      (nthcdr (1- i) (cdr l))))
  (define (ncons i v l) ; cons an element i times to the front of a list
    (if (<= i 0)
      l
      (ncons (1- i) v (cons v l))))
  (define (collapse-accumulator i n a) ; iteratively collapse all accumulators together
    (if (= i n)
      (cond ((= i 0) (cond ((null? (cdar a)) (fix 'vect (car a)))           ; [x] => (vect x)
                           (else             (fix 'hcat (car a))))          ; [x y] => (hcat x y)
            ((= i 1)                         (fix 'vcat (car a)))           ; [x;y] => (vcat x y) and [x y;a b] => (vcat (row x y) (row a b))
            (else                            (fix 'ncat (cons (1+ i) a))))) ; [x;y;;a;b] => (ncat 3 (vcat x y) (vcat a b))
      (let ((ah (car a)))
        (let ((ahnext (cadr a)))
          (let ((ahnexth (cond ((= i 0) (cond ((null? (cdr ah)) ah)
                                              (else             (fix 'row ah)))
                               ((= i 1)                         (fix 'vcat ah))
                               (else                            (fix 'ncat (cons (1+ i) ah)))))))
            (set! a (cons (cons ahnexth ahnext) (cddr a)))
            (collapse-accumulator (1+ i) n a))))))
  (define (parse-ndim-matrix-inner s a semicolon-count max-level closer gotnewline last-end-symbol)
   (let ((t (require-token s)))
     (if (eqv? t closer)
       (collapse-accumulator 0 max-level a)
       (case t
         ((#\;)
           (take-token s)
           (let ((ant (nthcdr semicolon-count a)))
             (let ((an (car ant)))
               (error list(ant an))
               (let ((annext (cond ((null? (cdr ant)) '())
                                   (else              (cadr ant)))))
                 (set! semicolon-count (1+ semicolon-count))
                 (let ((annexth (cond ((= semicolon-count 1) (cond ((not (pair? an)) an)
                                                                   ((null? (cdr an)) an)
                                                                   (else             (fix 'row an))))
                                      ((= semicolon-count 2) (fix 'vcat an))
                                      (else                  (fix 'ncat (cons semicolon-count an))))))
                   (let ((at (cond ((null? (cdr ant)) '())
                                   (else              (cddr ant)))))
                     (set! a (ncons semicolon-count '() (cons (cons annexth annext) at)))
                     (parse-ndim-matrix-inner s a semicolon-count (max max-level semicolon-count) closer gotnewline last-end-symbol)))))))
         ((#\,)
           (error "unexpected comma in matrix expression"))
         ((#\] #\})
          (error (string "unexpected \"" t "\"")))
         ((for)
           ; put comprehension code here
         )
         (else
           (let ((u (parse-eq* s)))
             (parse-ndim-matrix-inner s (list (list u (car a)) (cdr a)) 0 max-level closer gotnewline last-end-symbol)))))))
  (error (parse-ndim-matrix-inner s (list first) 0 0 closer gotnewline last-end-symbol)))

(define (parse-ndim-matrix-one s first closer gotnewline last-end-symbol)
  (define (fix head v) (cons head (reverse v)))
  (define (update-outer v outer)
              ...
              (if issemicolon
                (begin (set! semicolon-count (1+ semicolon-count))
                       (if (> semicolon-count 1)
                         (begin (set! outer (cons 'semicolon outer))
                                (error outer))))
                ;(begin (set! semicolon-count (1+ semicolon-count))
                ;       (if (> semicolon-count 1)
    ...
    (if (eqv? (require-token s) closer)
        ...
        (let* ((first (parse-eq* s))
          ...
          (cond ((or (eqv? t #\,) (eqv? t closer))
                ...
                (else
                 (parse-ndim-matrix s first closer #f last-end-symbol)))))))
