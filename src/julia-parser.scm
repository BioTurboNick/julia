...
(define (parse-ndim-matrix s first closer gotnewline last-end-symbol)
  (define (fix head v) (cons head (reverse v)))
  (define (parse-ndim-matrix-inner s a r semicolon-count max-semicolon-count row-count closer gotnewline last-end-symbol)
    (let ((t (require-token s)))
      (if (eqv? t closer)
        (begin (take-token s)
               (cond ((null? r)       a)
                     ((null? (cdr r)) (cons r a))
                     (else            (cons (fix 'row r) a))
               )
        )
        (case t
          ((#\;)
            ...
            (if (= semicolon-count 0)
              (parse-ndim-matrix-inner s (cons (fix 'row r) a) '() (1+ semicolon-count) (max max-semicolon-count (1+ semicolon-count)) 0 closer gotnewline last-end-symbol)
              (parse-ndim-matrix-inner s (cons (fix 'vcat a) '() (1+ semicolon-count) (max max-semicolon-count (1+ semicolon-count)) 0 closer gotnewline last-end-symbol)  ;;; working on this, next dimension
            )
          )
          ((#\,)
            (error "unexpected comma in matrix expression")
          ...
          (else
            (let ((u (parse-eq* s)))
              (if (null? r)
                (parse-ndim-matrix-inner s a (list u) 0 max-semicolon-count (1+ row-count) closer gotnewline last-end-symbol)
                (parse-ndim-matrix-inner s a (cons u r) 0 max-semicolon-count (1+ row-count) closer gotnewline last-end-symbol)
              )
            )
          )
        )
  ...
  (error (parse-ndim-matrix-inner s '() (list first) 0 0 1 closer gotnewline last-end-symbol))
)
