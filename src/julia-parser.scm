...
(define (parse-ndim-matrix s first closer gotnewline last-end-symbol)
  (define (fix head v) (cons head (reverse v)))
  (define (fixcat head dims v) (cons head (cons dims (reverse v))))
  (define (ncons i v l) ; cons an element v i times to the front of a list
    (if (<= i 0)
      l
  ...
  (define (nthcdr i l) ; get the list i elements back
    (if (<= i 0)
      l
      (nthcdr (1- i) (cdr l))))
  (define (fix-level i v)
    (case i
      (0 ; only true if no semicolons encountered when closer reached
        (cond ((null? (cdr v)) (fix 'vect (car v)))   ; [x]   => vect x
              (else            (fix 'hcat v)))) ; [x y] => hcat x y
      (1
  ...
  (define (collapse-levels i n a)
    (if (= n 0)
      ...
      (let ((inext (1+ i)))
        (if (= i n)
          (fix-level inext (car a))
          (begin (let ((afix (fix-level inext (car a))))
                      ;(set! a (cond ((= inext n) (cons afix (cdr a)))
                      ;              (else    (cons (cons afix (cadr a)) (cddr a))))))
                      (set! a (cons (cons afix (cadr a)) (cddr a))))
                 (collapse-levels inext n a))))))
  (define (parse-ndim-matrix-inner s a semicolon-count max-level closer gotnewline)
    (let ((t (require-token s)))
      (if (eqv? t closer)
        (begin (take-token s)
               (collapse-levels 0 max-level a))
        (case t
          ((#\;)
            (take-token s)
            (let ((ant (nthcdr semicolon-count a)))
              (set! semicolon-count (1+ semicolon-count))
              (let ((afix (fix-level semicolon-count (car ant))))
                ;(if (= max-level 1)
                ;  (error (list afix semicolon-count max-level (cons afix (cadr ant)) (cddr ant) (cons (cons afix (cadr ant)) (cddr ant)))))
                (set! a (cond ((< semicolon-count max-level) (cons (cons afix (cadr ant)) (cddr ant)))
                              ((= semicolon-count max-level) (cons (cons afix (cadr ant)) (cddr ant)))
                              ((> semicolon-count max-level) (cons (list afix) '()))))
                ))
            ...
            (parse-ndim-matrix-inner s a semicolon-count max-level closer gotnewline))
          ((#\,)
            (error "unexpected comma in matrix expression"))
          ((#\] #\})
            (error (string "unexpected \"" t "\"")))
          ...
          (else
            (let ((u (parse-eq* s)))
              ;(if (= max-level 1)
              ;  (error (cons (cons u (car a)) (cdr a)))
              ;)
              (parse-ndim-matrix-inner s (cons (cons u (car a)) (cdr a)) 0 max-level closer gotnewline)))))))
   (parse-ndim-matrix-inner s (cons (list first) '()) 0 0 closer gotnewline))
